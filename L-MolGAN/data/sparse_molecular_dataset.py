...
class SparseMolecularDataset():
    ...
    def generate(self, filename, add_h=False, filters=lambda x: True, size=None, validation=0.05, test=0.05):
        ...
        if filename.endswith('.sdf'):
            ...
            self.data = [
                ...
                if (mol := Chem.MolFromSmiles(line.strip())) is not None
            ]
...
if __name__ == '__main__':
    ...
    # data.generate('zinc_clean_4.4.smi', filters=lambda x: x.GetNumAtoms() <= 9)
    # data.save('zinc_clean_4.9.sparsedataset')

    data = SparseMolecularDataset()
    data.generate('gdb9_clean_4.4.smi', filters=lambda x: x.GetNumAtoms() <= 9)
    data.save('gdb_9.sparsedataset')

    # data = SparseMolecularDataset()
    # data.generate('zinc.smi', validation=0.00021, test=0.00021)  # , filters=lambda x: x.GetNumAtoms() <= 9)
    # data.save('zinc.sparsedataset')
