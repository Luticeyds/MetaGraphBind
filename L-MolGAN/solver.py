    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            if (i+1) % self.n_critic == 0:
                ...
                # Real Reward
                rewardR = torch.from_numpy(self.reward(mols, self.data)).to(self.device)
                # Fake Reward
                (edges_hard, nodes_hard) = self.postprocess((edges_logits, nodes_logits), 'hard_gumbel')
                edges_hard, nodes_hard = torch.max(edges_hard, -1)[1], torch.max(nodes_hard, -1)[1]
                mols = [self.data.matrices2mol(n_.data.cpu().numpy(), e_.data.cpu().numpy(), strict=True)
                        for e_, n_ in zip(edges_hard, nodes_hard)]
                rewardF = torch.from_numpy(self.reward(mols, self.data)).to(self.device)
