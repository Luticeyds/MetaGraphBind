...
def density_graph(true_values, predicted_values, r2 = 0.0, net_name = 'net', mode = 'train', save = False):
    ...
    plt.tight_layout()
    plt.savefig('net/' + net_name + '.png', dpi=1000, bbox_inches="tight")
    # plt.show() # Uncomment to display the plot
    plt.close()
...
def save_pooled_graphs_as_png(smiles_list, prefix, num, molsPerRow=1):
    ...
    plt.imshow(img)
    plt.axis('off')
    plt.savefig(f'./net/{prefix}/{num}.png', dpi=1000)
    plt.close()
...
def save_fp_contribution(data, filename):
    ...
    plt.savefig('net/' + filename, dpi=1500)
    plt.close()
